...
REPORT_PATH = os.path.join(REPORT_DIR, 'without_retail_price_model_report.txt')
...
def get_feature_list_without_retail_price():
    return [
...
def build_report_text(metrics):
    lines = []
    lines.append('Without Retail Price Features Model Report')
    lines.append('=' * 52)
    lines.append('')
    lines.append('Target: retail_price')
    lines.append('Feature set excludes retail_price-derived predictors.')
...
def run_without_retail_price_model():
    ...
    print('\n' + '=' * 80)
    print('WITHOUT RETAIL PRICE FEATURES MODEL')
    print('=' * 80)
...
if __name__ == '__main__':
    run_without_retail_price_model()