...
REPORT_PATH = os.path.join(REPORT_DIR, 'single_model_without_mean_farmer_price_report.txt')
...
def get_feature_list_without_mean_farmer_price():
    return [
...
def prepare_dataset(data_path):
    ...
    df.drop(columns=['mean_farmer_price'], inplace=True, errors='ignore')
...
def build_report_text(metrics, study_xgb, study_lgb, weights):
    lines = []
    lines.append('Single Model Report Without Mean Farmer Price')
    lines.append('=' * 50)
    lines.append('')
    lines.append('Feature set excludes mean_farmer_price and all engineered features derived from it.')
    lines.append('Accuracy is defined as 1 - MAPE.')
    lines.append('')
    lines.append('Holdout Metrics')
    lines.append('---------------')
    lines.append(f"R2       : {metrics['r2']:.4f}")
    lines.append(f"Accuracy : {format_pct(metrics['accuracy'])}")
    lines.append(f"MAPE     : {format_pct(metrics['mape'])}")
    lines.append('')
    lines.append('Tuned Parameters')
    lines.append('-----------------')
    lines.append(f"XGBoost  : {study_xgb.best_params}")
    lines.append(f"LightGBM : {study_lgb.best_params}")
    lines.append('')
    lines.append('Ensemble Weights')
    lines.append('----------------')
    lines.append(f"XGBoost Weight : {weights['xgb']:.4f}")
    lines.append(f"LightGBM Weight : {weights['lgb']:.4f}")
    return '\n'.join(lines)


def run_single_model_without_mean_farmer_price():
    os.makedirs(REPORT_DIR, exist_ok=True)

    df_ready = prepare_dataset(DATA_PATH)
    train_df, test_df = make_train_test_split(df_ready)

    features = get_feature_list_without_mean_farmer_price()
    run_result = tune_xgb_lgbm_and_weights(train_df, test_df, features)

    metrics = run_result['holdout_results']
    report_text = build_report_text(metrics, run_result['study_xgb'], run_result['study_lgb'], run_result['weights'])

    print('\n' + '=' * 80)
    print('SINGLE MODEL WITHOUT MEAN FARMER PRICE')
    print('=' * 80)
...
if __name__ == '__main__':
    run_single_model_without_mean_farmer_price()
