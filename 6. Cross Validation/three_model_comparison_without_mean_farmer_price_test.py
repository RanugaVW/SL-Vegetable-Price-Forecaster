...
REPORT_PATH = os.path.join(REPORT_DIR, 'three_model_comparison_without_mean_farmer_price_report.txt')
...
def get_feature_sets():
    ...
    mean_farmer_price_features = {
        'mean_farmer_price_filled',
        'farmer_retail_spread_lag_1',
        'mean_farmer_price_lag_1',
        'mean_farmer_price_lag_2',
        'mean_farmer_price_lag_3',
        'mean_farmer_price_lag_4',
        'mean_farmer_price_lag_5',
        'mean_farmer_price_lag_6',
        'mean_farmer_price_lag_8',
        'farmer_price_roll_4',
        'farmer_price_roll_8',
        'farmer_price_roll_std_4',
        'farmer_price_pct_change_1',
        'farmer_price_momentum_1_4',
    }

    no_mean_farmer_price_features = [
        feature for feature in full_features
        if feature not in mean_farmer_price_features
    ]
    ...
    return {
        ...
        'Without Mean Farmer Price': no_mean_farmer_price_features,
    ...
    rows = [
        {
            ...
            'Without Mean Farmer Price': f"{results_by_model['Without Mean Farmer Price']['r2']:.4f}",
            'Without Diesel': f"{results_by_model['Without Diesel']['r2']:.4f}",
        },
        {
            'Metric': 'Accuracy',
            'Full Model': format_pct(results_by_model['Full Model']['accuracy']),
            'Without Mean Farmer Price': format_pct(results_by_model['Without Mean Farmer Price']['accuracy']),
            'Without Diesel': format_pct(results_by_model['Without Diesel']['accuracy']),
        },
        {
            'Metric': 'MAPE',
            'Full Model': format_pct(results_by_model['Full Model']['mape']),
            'Without Mean Farmer Price': format_pct(results_by_model['Without Mean Farmer Price']['mape']),
...
def build_report_text(table_df):
    lines = []
    lines.append('Three-Model Comparison Report - Mean Farmer Price Importance')
    lines.append('=' * 66)
    lines.append('')
    lines.append('Models compared: Full Model, Without Mean Farmer Price, Without Diesel.')
